/*
 *	OKUAS -- A Simple 8086 Tool Chain
 *	8086 assembler
 *
...
void ckpubs(void) {
	...
	for (i=0; i<Symtop; i+=SSIZE) {
		if (Symbols[i+SCLASS] == PUBLIC)
			...
			printf("okuas: undefined public symbol: %s\n",
				&Symbols[i]);
			Errcnt++;
		}
	}
...
usage() {
	printf("OKU Assembler (2026); Nils M Holm, 1993,1995,2013,2014\nusage: okuas [-dv] [-o file] { file | - } ...\n");
	exit(1);
}
