...
static void compile(char *file, char *def) {
	...
	if (O_verbose) {
		if (O_verbose > 1)
			printf("okucc.exe %s %s\n", file, ofile);
		else
			printf("compiling %s\n", file);
	}
	if (strlen(file)*2 + (def? strlen(def): 0) + 16 > 128)
		cmderror("file name too long", file);
	sprintf(okucc_cmd, "okucc.exe %s %s %s", file, ofile, def? def: "");
	if (system(okucc_cmd))
...
static void link(void) {
	...
	if (strlen(ofile) + strlen(LDCMD) + strlen(OKUCCDIR)*2 >= TEXTLEN)
		cmderror("linker command too long", NULL);
	sprintf(cmd, LDCMD, ofile, OKUCCDIR, O_stdio? "": "n");
	k = strlen(cmd);
	for (i=0; i<Nf; i++)
		k = concat(k, cmd, Files[i]);
	k = concat(k, cmd, OKUCCLIBC);
	concat(k, cmd, SYSLIBC);
	sprintf(cmd2, cmd, OKUCCDIR);
	if (O_verbose > 1) printf("%s\n", cmd2);
	if (system(cmd2))
...
static void usage(void) {
	printf("The OKU C Compiler (2026); NMH's Simple C Compiler, 2011--2014\nUsage: okucc [-h] [-ctvNSV] [-d opt] [-o file] [-D macro[=text]]"
		" file [...]\n");
}
...
static void version(void) {
	printf("OKUCC version %s for %s/%s\n", VERSION, OS, CPU);
}
